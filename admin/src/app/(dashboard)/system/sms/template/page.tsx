...
export default function SmsTemplateList() {
  ...
  // Load channels for dropdown selector
  ...
  const handleTestSend = async (values: any) => {
    ...
    try {
      ...
      const res = await fetch(`http://localhost:3000/admin-api/system/sms/template/${testTemplateId}/send-mock`, {
        method: 'POST',
        headers: {
      ...
      message.success('已触发模拟短信发送，请查看日志！');
    ...
    <div style={{ padding: '24px' }}>
      ...
      >
        ...
        >
          ...
          <Form.Item name="channelId" label="短信渠道" rules={[{ required: true, message: '请选择短信渠道' }]}>
            <Select placeholder="请选择短信渠道" {...channelSelectProps} />
          </Form.Item>

          <Form.Item name="code" label="模板编码" rules={[{ required: true, message: '请输入模板编码 (如 sms_login)' }]}>
          ...
          <Form.Item name="content" label="模板内容" rules={[{ required: true, message: '请输入模板内容' }]}>
            <Input.TextArea rows={3} placeholder="请输入模板内容，如：您的登录验证码是 {code}，有效期5分钟。" />
          </Form.Item>
      ...
      >
        ...
        >
          <Form.Item name="mobile" label="手机号码" rules={[{ required: true, message: '请输入测试接收号码' }]}>
