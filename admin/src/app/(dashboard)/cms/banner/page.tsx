...
export default function CmsBannerList() {
  ...
  const handleSubmit = async (values: any) => {
    ...
    try {
      if (editingId) {
        await axiosInstance.put(`/cms/banner/${editingId}`, values);
        message.success('修改成功');
      } else {
        await axiosInstance.post('/cms/banner', values);
    ...
    <div style={{ padding: '24px' }}>
      ...
      >
        <Table {...tableProps} rowKey="id">
          ...
          <Table.Column
            ...
            title="图片"
            width={120}
            render={(url: string) => url ? <img src={url} alt="banner" style={{ width: 80, height: 40, objectFit: 'cover', borderRadius: 4 }} /> : '-'}
          />
          <Table.Column dataIndex="url" title="跳转链接" ellipsis />
      ...
      >
        <Form form={form} layout="vertical" onFinish={handleSubmit}>
          ...
          <Form.Item name="picUrl" label="图片URL" rules={[{ required: true, message: '请输入图片URL' }]}>
