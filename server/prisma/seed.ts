...
async function main() {
  ...
  // Categories
  const catTech = await prisma.cmsCategory.create({ data: { name: '技术文章', code: 'tech', sort: 1, status: CommonStatus.ENABLE, remark: '技术相关文章' } });
  const catNews = await prisma.cmsCategory.create({ data: { name: '新闻资讯', code: 'news', sort: 2, status: CommonStatus.ENABLE, remark: '新闻资讯类文章' } });
  const catProduct = await prisma.cmsCategory.create({ data: { name: '产品动态', code: 'product', sort: 3, status: CommonStatus.ENABLE, remark: '产品更新和动态' } });
  const catActivity = await prisma.cmsCategory.create({ data: { name: '活动公告', code: 'activity', sort: 4, status: CommonStatus.ENABLE, remark: '活动和公告' } });
  const catTutorial = await prisma.cmsCategory.create({ data: { name: '使用教程', code: 'tutorial', sort: 5, status: CommonStatus.ENABLE, remark: '产品使用教程' } });

  // Tags
  const tagNestjs = await prisma.cmsTag.create({ data: { name: 'NestJS', status: CommonStatus.ENABLE } });
  const tagReact = await prisma.cmsTag.create({ data: { name: 'React', status: CommonStatus.ENABLE } });
  const tagPrisma = await prisma.cmsTag.create({ data: { name: 'Prisma', status: CommonStatus.ENABLE } });
  const tagTypeScript = await prisma.cmsTag.create({ data: { name: 'TypeScript', status: CommonStatus.ENABLE } });
  const tagAntd = await prisma.cmsTag.create({ data: { name: 'Ant Design', status: CommonStatus.ENABLE } });
  const tagDocker = await prisma.cmsTag.create({ data: { name: 'Docker', status: CommonStatus.ENABLE } });
  const tagPostgres = await prisma.cmsTag.create({ data: { name: 'PostgreSQL', status: CommonStatus.ENABLE } });
  const tagSecurity = await prisma.cmsTag.create({ data: { name: '安全', status: CommonStatus.ENABLE } });

  // Articles (30 articles for pagination testing)
  const articleStatuses = ['PUBLISHED', 'PUBLISHED', 'PUBLISHED', 'DRAFT', 'ARCHIVED'] as const;
  ...
  const articleContents = articleTitles.map((title, i) =>
    `<h2>${title}</h2><p>这是关于「${title}」的详细文章内容。</p><p>在本文中，我们将深入探讨这个话题的各个方面，包括核心概念、最佳实践和常见陷阱。</p><p>第 ${i + 1} 段：通过实际案例来说明这些概念的应用。</p><p>总结：掌握这些知识将帮助你在项目中做出更好的技术决策。</p>`,
  );
  ...
  const commentStatuses = ['APPROVED', 'APPROVED', 'PENDING', 'REJECTED'] as const;
  for (let i = 0; i < 20; i++) {
  ...
  for (let i = 0; i < 8; i++) {
    await prisma.cmsBanner.create({
      data: {
        ...
        sort: i,
        status: i < 6 ? CommonStatus.ENABLE : CommonStatus.DISABLE,
        remark: `第 ${i + 1} 个轮播图`,
      },
    });
  }
