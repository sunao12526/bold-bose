...
let CodegenService = CodegenService_1 = class CodegenService {
    ...
    async importTables(tableNames, author = 'Antigravity') {
      ...
      `, tableName);
            if (tableInfo.length === 0) {
                throw new common_1.NotFoundException(`数据库表 ${tableName} 未找到`);
            }
    ...
    mapPostgresType(dataType, columnName) {
    ...
    getDefaultHtmlType(columnName, tsType, isPrimaryKey) {
        if (isPrimaryKey) {
    ...
    async findOneTable(id) {
        ...
        if (!table)
            throw new common_1.NotFoundException('导入表记录不存在');
        return table;
    }
    async updateTableMetadata(id, data) {
    ...
    async generateCodePreview(tableId) {
    ...
    async registerInParentModule(baseDir, moduleName, businessName, className) {
...
`;
    ...
    renderService(table) {
        ...
        let filterLogics = '';
        const searchColumns = table.columns.filter((c) => c.listOperation && !c.primaryKey);
